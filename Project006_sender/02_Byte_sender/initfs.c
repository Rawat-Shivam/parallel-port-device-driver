...
#include"fileOps.h"
	 ...
	 *
	 * In character driver we are just moving data between kernel space and user space...
	 * Never ever transfer any data to real/physical device..... Here we are first time transmitting data 
	 * between 2 computer using IO ports .... whose exclusive access is acquired through request_region()
	 */
...
RET:
		...
		else
		{
			...
			// sending data directly to parallel port using outb() to verify hardware connection of device
			outb(1,BASE_ADDR);
			ssleep(2);
			outb(2,BASE_ADDR);
			ssleep(2);
